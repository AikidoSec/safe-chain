...
set -g _sc_shims_dir "$safe_chain_base/shims"
...
function removeSafeChainShimFromPath
    set -l path_without_shims

    for path_entry in $PATH
        if test "$path_entry" != "$_sc_shims_dir"
            set -a path_without_shims "$path_entry"
        end
    end

    string join : $path_without_shims
end
...
function wrapSafeChainCommand
    ...
    if type -q safe-chain
        ...
        # Remove safe-chain shims from PATH so safe-chain resolves the real command
        # instead of re-entering a shim generated by setup-ci.
        env -u PKG_EXECPATH PATH=(removeSafeChainShimFromPath) safe-chain $original_cmd $cmd_args
    else
