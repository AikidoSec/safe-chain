...
describe("E2E: pyenv and safe-chain shims interplay", () => {
   ...
   * PATH is not touched here; each test arranges its own PATH afterwards.
   */
  async function setupPyenvWithSafeChainShims(shell) {
   ...
   */
  async function assertPyenvResolvesToSafeChainShim(shell) {
  ...
  it("python3 runs when pyenv resolves it through the safe-chain shim", async () => {
    ...
    await setupPyenvWithSafeChainShims(shell);
    await shell.runCommand(
      'export PATH="$PYENV_ROOT/shims:/root/.safe-chain/shims:$PATH"'
    );
    await assertPyenvResolvesToSafeChainShim(shell);
  ...
  it("python3 runs when PATH spells the shims dir with a trailing slash", async () => {
    ...
    // exact "dir:" spelling, so "dir/:" kept the shim reachable and python
    // looped forever between the pyenv and safe-chain shims.
    const shell = await container.openShell("bash");
    await setupPyenvWithSafeChainShims(shell);
    await shell.runCommand(
      'export PATH="$PYENV_ROOT/shims:/root/.safe-chain/shims/:$PATH"'
  ...
  it("python3 terminates when the shims dir is the last PATH entry", async () => {
    // Regression: the old sed-based PATH cleanup required a trailing colon, so
    // the shims dir as the LAST PATH entry was never removed and python looped
    // forever. With no real python3 on PATH the fixed shim chain must end in
    // pyenv's clean "command not found" instead of a hang.
    const shell = await container.openShell("bash");
    await setupPyenvWithSafeChainShims(shell);
    // Hide the real python3 binaries (after rehash, so the fake version's
    // symlink resolved) — the safe-chain shim as last PATH entry must be the
    // only python3 that pyenv's system fallback can find
    await shell.runCommand(
      "rm -f /usr/local/bin/python3 && mv /usr/bin/python3 /usr/bin/hidden-python3"
    );
    await shell.runCommand(
      'export PATH="$PYENV_ROOT/shims:$PATH:/root/.safe-chain/shims"'
    );
    await assertPyenvResolvesToSafeChainShim(shell);

    const result = await shell.runCommand("python3 --version; echo EXIT:$?");
    assert.ok(
      result.output.includes("command not found"),
      `Expected a clean "command not found" (not an exec loop / hang), output was: ${result.output}`
    );
    assert.ok(
      !result.output.match(/EXIT:0/),
      `Expected a non-zero exit code, output was: ${result.output}`
    );
  });
});
