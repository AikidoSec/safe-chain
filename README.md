...
```shell
curl -fsSL https://github.com/AikidoSec/safe-chain/releases/download/1.5.19/install-safe-chain.sh -o /tmp/install-safe-chain.sh \
  && echo "537a4c365c82e43e276e13bbfaecdb79b41d993d654b30d00edcaecfe20dd3ee  /tmp/install-safe-chain.sh" | sha256sum -c - \
...
Invoke-WebRequest "https://github.com/AikidoSec/safe-chain/releases/download/1.5.19/install-safe-chain.ps1" -OutFile $installer -UseBasicParsing
$expectedHash = "30326CB5EFE16ACA437A40E9B899D9DB0E830E863FC0CE22FC224A77920EB280"
...
The install scripts are served from a versioned release URL (`releases/download/1.5.19/...`). GitHub releases are immutable — once an artifact is published at a versioned URL it cannot be modified or replaced, so the file you download is guaranteed to be exactly what was released.
...
```shell
curl -fsSL https://github.com/AikidoSec/safe-chain/releases/download/1.5.19/uninstall-safe-chain.sh | sh
```

### Windows (PowerShell)

```powershell
iex (iwr "https://github.com/AikidoSec/safe-chain/releases/download/1.5.19/uninstall-safe-chain.ps1" -UseBasicParsing)
```
...
1. Define a dockerfile to run your build
   ...
   # Install safe-chain
   RUN curl -fsSL https://github.com/AikidoSec/safe-chain/releases/download/1.5.19/install-safe-chain.sh -o /tmp/install-safe-chain.sh \
     && echo "537a4c365c82e43e276e13bbfaecdb79b41d993d654b30d00edcaecfe20dd3ee  /tmp/install-safe-chain.sh" | sha256sum -c - \
     && sh /tmp/install-safe-chain.sh --ci \
     && rm /tmp/install-safe-chain.sh
